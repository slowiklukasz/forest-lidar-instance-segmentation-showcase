...
- **Budget-aware by design:** point clouds are density-downsampled to a fixed points/m² budget before training/inference, matching what budget cloud GPUs (Paperspace Gradient) can actually process. This is a deliberate scope choice — the goal is a model that **generalizes robustly at practical density**, not one that chases maximum achievable resolution on narrow, compute-heavy sub-problems (e.g. separating individual tangled/overlapping branches). Trade real-world usability and reproducibility on modest hardware over marginal gains that only show up on expensive setups.
